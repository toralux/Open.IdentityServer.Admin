...
20. Client wizard for the high secure client type: starts with a shared secret, warns about it and enforces the DPoP settings
21. Client `Integration` tab: generated setup code follows the unsaved form, grant types, scopes and the tab's own settings
...
- `tests/client-integration.spec.ts` - setup code generation on the client `Integration` tab; changes the form without saving
...
- `E2E_ADMIN_ROLE` (default: `ToraluxIdentityAdminAdministrator`)
